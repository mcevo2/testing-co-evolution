...
public class JavaVisitor extends ASTVisitor {//abstract?
	...
	/* TO take into account the applied resolutions which may influence the remaining resolutions we have to 
	...
	public boolean visit(SimpleName node) {//TODO if the binding treatment is needed elsewhere, then extract it in a new method
		...
		if(binding == null) {
			System.out.println("  Binding null  "+node);
		}
		else {

			if(!JavaParser.isPrimitiveType(binding)){
				if(binding.getKey().contains("ArrayIterable"))
				{
					System.out.println(" Arrayiterable nodes "+node+ " its binding "+node.resolveBinding().getKey()+ " starts at  "+node.getStartPosition());
				}
				if(manageBindings.getBindingsNodes().containsKey(binding)){
					...
					//System.out.println("  adding binding success  "+binding+" the key  "+binding.getKey()+ "  the node "+ node);
					/*if( binding.getKey().equals("Lorg/eclipse/ocl/pivot/utilities/ArrayIterable<Ljava/lang/Object;>;"))
					{
						System.out.println("  in wanted binding of usage 2 ");
					}
					*/
